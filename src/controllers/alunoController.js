...
// --- BANCO DE DADOS EM MEMÓRIA (Ativo) ---
let alunosBanco = [];
let contadorId = 1;
// -----------------------------------------
...
const criarAluno = async (req, res) => {
    try {
        ...
        alunosBanco.push(novoAluno);
        contadorId++;
        return res.status(201).json({ ...novoAluno, mensagem: "Aluno cadastrado em memória com sucesso!" });
...
const listarAlunos = async (req, res) => {
    try {
        // --- LÓGICA EM MEMÓRIA (Ativa) ---
        return res.status(200).json(alunosBanco);
...
const atualizarAluno = async (req, res) => {
    try {
        ...
        // --- LÓGICA EM MEMÓRIA (Ativa) ---
        const index = alunosBanco.findIndex(aluno => aluno.id === id);
        
        if (index === -1) {
            return res.status(404).json({ erro: "Aluno não encontrado." });
        }

        alunosBanco[index] = { id, nome, matricula, curso };
        return res.status(200).json({ mensagem: "Dados atualizados em memória com sucesso!" });
...
const deletarAluno = async (req, res) => {
    try {
        ...
        alunosBanco = alunosBanco.filter(aluno => aluno.id !== id);
        return res.status(200).json({ mensagem: "Aluno removido da memória com sucesso!" });
